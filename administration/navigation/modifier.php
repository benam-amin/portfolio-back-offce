...
<head>
    ...
    <title>Modification - Navigation</title>
...
<body class="bg-gray-100 text-gray-900">
    <?php 
        ...
        if ($entree_mise_a_jour) { //s'il y a bien un id à modifier
            ...
            $resultat_affiche = genererRequeteEtResultat("navigation", $id, $connexion_bdd); //récupère la requête est le résultat d'affichage
            $entite = mysqli_fetch_assoc($resultat_affiche); //stock le retour dans la variable entité
        }

        if ($formulaire_soumis) { 
            if (!empty($_POST["ancre"]) && !empty($_POST["contenu"])) { //vérification de l'entrée des input du formulaire
                ...
                $requete_modif = "UPDATE navigation SET ancre = '$ancre', contenu = '$contenu', visibilite = $visibilite WHERE id = $id;"; //requête de modification
                    ...
                    <form method="POST" action="">
                        <section class="grid gap-6">
                            <div>
                                ...
                                <input type="text" value="<?= htmlspecialchars($entite['ancre']); ?>" name="ancre" id="ancre" 
                            ...
                            <div>
                                ...
                                <input type="text" value="<?= htmlspecialchars($entite['contenu']) ; ?>" name="contenu" id="contenu" 
                                    class="mt-1 block w-full rounded-md py-2 border-gray-300 shadow-sm focus:border-gray-800 focus:ring-gray-800">
                            </div>
